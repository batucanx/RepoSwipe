        ...
        override suspend fun doWork(): Result {
            ...
            for (entry in pending) {
                try {
                    ...
                    // Anything else — including a 403 the token's scope can't satisfy — fails
                    // identically on every retry. Dropping it (and reporting it, since this is
                    // the one place a star/unstar can silently never reach GitHub) keeps a
                    // permanently un-syncable entry from pinning the row's optimistic state forever.
